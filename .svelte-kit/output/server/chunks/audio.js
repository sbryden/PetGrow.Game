...
const sounds = {
  inject() {
    ...
    g.gain.exponentialRampToValueAtTime(1e-3, ac.currentTime + 0.3);
    osc.connect(g);
    g.connect(ac.destination);
    osc.start();
  ...
  clean() {
    ...
    lp.type = "lowpass";
    lp.frequency.value = 2e3;
    g.gain.setValueAtTime(0.1, ac.currentTime);
    g.gain.exponentialRampToValueAtTime(1e-3, ac.currentTime + 0.25);
    n.connect(lp);
    lp.connect(g);
    g.connect(ac.destination);
    n.start();
    ...
    setTimeout(() => tone(ac, 1e3, "sine", 0.1, 0.07), 250);
  },
  play() {
    const ac = getCtx();
    const osc = ac.createOscillator();
  ...
  room() {
    ...
    g.gain.exponentialRampToValueAtTime(1e-3, ac.currentTime + 0.2);
    n.connect(bp);
    bp.connect(g);
    g.connect(ac.destination);
    n.start();
