<script>
  ...
  // ── Local state ───────────────────────────────────────────
  let petName = $state('');
  let ingredients = $state({ animal: null, color: null, wildcard: null, element: null });
  let syringeVisible = $state(false);
  let syringeColor = $state('#00d2ff');
  let syringeAnimating = $state(false);
...
<div class="egg-lab screen">
  ...
  <div class="hatch-section">
    <p class="hatch-hint" style="color: {hint.color || 'inherit'}">{hint.text}</p>
